...
def _serialize_message(message: discord.Message) -> dict:
    return {
        ...
        "guild": message.guild and serialize_guild(message.guild),
        "tts": message.tts,
        "attachments": serialize_attachments(message)
    }


def serialize_channel(channel: discord.TextChannel) -> dict:
    return {
        "id": channel.id,
...
def serialize_user(user: Union[discord.User, discord.Member]) -> dict:
    return {
        "id": user.id,
        "username": user.name,
        "avatar": user.avatar,
...
def serialize_attachments(message: discord.Message):
...
def serialize_context(ctx: CustomContext) -> dict:
    return {
        "message": _serialize_message(ctx.message),
        "args": [default_serialization(x) for x in ctx.args if type(x) != commands.Context],
...
VALID_SERIALIZATIONS = {
    ...
    discord.User: serialize_user,
    discord.Member: serialize_user,
    discord.TextChannel: serialize_channel,
    discord.Guild: serialize_guild,
...
class Serialized:
    def __init__(self, error: Exception, ctx: CustomContext):
        ...
        self.frame = serialize_tb_frame(error.__traceback__)
